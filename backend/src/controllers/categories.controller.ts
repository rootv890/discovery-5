...
import { NewCategoryType, categories, categoryPlatform } from "../db/schema";
...
export async function createCategoryAndAddToPlatform(
	category: typeof NewCategoryType,
	platformId?: string
) {
	try {
		// Start the tx
		const transaction = await db.transaction(async (tx) => {
			...
			//   Insert the category platform
			if (platformId) {
				// add to categoryPlatform
				return await tx
			...
			// end
			return [newCategory, categoryPlatform];
		});
...
export const getAllCatsFromPlatform = async (platformId: String) => {
	...
	try {
		...
		const allCats = await db
			.select()
			.from(categoryPlatform)
			.where(eq(categoryPlatform.platformId, platformId as string));

		const allCatsId = allCats.map((cat) => cat.categoryId);
		console.table(allCatsId);

		const allCatsDetails = await db
			.select()
			.from(categories)
			.where(inArray(categories.id, allCatsId));
...
export const addCatToMultiplePlatForms = async ({
	catId,
	platformIds,
}: {
	catId: string;
	...
	try {
		...
		if (
			catExists[0].platformConstraint === "SINGLE" &&
			platformIds.length > 1
		) {
			throw new Error(
				"Category is single platform and adding to multiple platforms is Illegal"
			);
		}
		// Platforms Exists
		const platformsExists = await Promise.all(
			platformIds.map(async (id) => await doesPlatformExists(id))
		);
		...
		const catInPlatforms = await db
			.select()
			.from(categoryPlatform)
			.where(inArray(categoryPlatform.categoryId, [catId]));
		devConsole("table", "catInPlatforms", catInPlatforms);

		// Check for duplications same cat and plat
		const duplications = platformIds.filter((id) =>
			catInPlatforms.some((cat) => cat.platformId === id)
		);
		...
		const addCatToPlatforms = await db
			.insert(categoryPlatform)
			.values(
				uniquePlatformIds.map((id) => ({ categoryId: catId, platformId: id }))
			)
			.returning();
...
export const removeCatFromPlatforms = async ({
	catId,
	platformIds,
}: {
	catId: string;
	...
	try {
		...
		const catExists = await db
			.select()
			.from(categories)
			.where(eq(categories.id, catId.trim()));
		if (!catExists) {
			throw new Error("Category not found");
		}
